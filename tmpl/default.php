...
<div class="qldebugInner qldebug<?php echo ucwords($type);?>">
    <?php if(2!=$params->get('output',1)):?>
        <div id="qldebugButton" class="qldebugInner">
            <button id="qldebugHide" class="btn" onclick="document.getElementById('qldebugShow').style.display='block';document.getElementById('qldebugHide').style.display='none';document.getElementById('qldebugInnerContainer').style.display='none';"><?php echo JText::_('MOD_QLDEBUG_JSHIDE');?></button>
            <button id="qldebugShow" class="btn" style="display:none;" onclick="document.getElementById('qldebugShow').style.display='none';document.getElementById('qldebugHide').style.display='block';document.getElementById('qldebugInnerContainer').style.display='block';"><?php echo JText::_('MOD_QLDEBUG_JSSHOW');?></button>
        </div>
    <?php endif;?>
    <div id="qldebugInnerContainer" class="qldebugInner <?php if(2!=$params->get('output',1)) echo 'bare';?>">
        ...
        foreach ($obj_plugin->arrQldebug as $v):
            ...
            elseif ('server'==$v) { if(0<count($obj_plugin->$v)) require $obj_plugin->getLayoutPath('_default'); }
